...
#include "tensorflow/neuron/grappler/auto_mixed_precision.h"
...
#include "tensorflow/neuron/grappler/auto_mixed_precision_lists.h"
 ...
 public:
  AutoMixedPrecisionImpl(Cluster* cluster,
      ...
      : virtual_placer_(cluster->GetDevices()),
        ...
        mode_(mode),
        target_dtype_(mode_ == AutoMixedPrecisionMode::MKL ? DT_BFLOAT16
                                                            : DT_HALF) {}
  ...
  std::unique_ptr<AutoMixedPrecisionLists> get_mixed_precision_lists() const {
    switch (mode_) {
      case AutoMixedPrecisionMode::CUDA:
        return make_unique<AutoMixedPrecisionListsCuda>(cuda_version_,
                                                             cudnn_version_);
      case AutoMixedPrecisionMode::MKL:
        return make_unique<AutoMixedPrecisionListsMkl>();
      
      case AutoMixedPrecisionMode::NEURON:
        return make_unique<AutoMixedPrecisionListsCuda>(0,0);
    }
  }
...
NodeDef AutoMixedPrecisionImpl::BuildCastNode(
  ...
  node.clear_input();
  node.add_input(strings::StrCat(src.node->name()));
  node.clear_attr();
